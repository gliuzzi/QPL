...
res2  = open("results10.txt","a")
@printf(res2, "----------------+-----------------+---------+-----------------+--------+---------\n")
@printf(res2, "PROBLEM         |                 |     GAP |             GUB |  NODES |    TIME\n")
@printf(res2, "----------------+-----------------+---------+-----------------+--------+---------\n")
@printf(res2, "                 con strong Bound tightening su tutti i nodi\n")
@printf(res2, "----------------+-----------------+---------+-----------------+--------+---------\n")
close(res2)
...
for n in dims
	...
	for i1 in 1:4
		for i2 in 1:4
			...
			begin
				(rootnodegap_time, GAP, prob2, LBs2, solved, error) = compute_rootnode_gap_10(DATA, nloc, MAXNODES, TOLGAP, time_limit)
				GUB = prob2.GUB
				totnodes = prob2.totnodes
				execution_time = 0.0
				if (!solved) && (!error)
					(GUB,GAP,totnodes,numlp,nummilp,execution_time,time_gap) = solve_10(DATA,prob2,LBs2,MAXNODES,TOLGAP,time_limit)
				end
				execution_time += rootnodegap_time
				res  = open("results10.txt","a")
				@printf(res, "%-15s | %-15s | %7.5f | %15.6e | %6d | %7.2f\n",probfile,"FORM(10)",GAP,GUB,totnodes,execution_time)
				close(res)
			end
		end
	end

...
time_limit = 1800
	...
	(rootnodegap_time, gap, prob7, LBs7) = compute_rootnode_gap_7(DATA, nloc, MAXNODES, TOLGAP, time_limit)
	(GUB,GAP,totnodes,numlp,nummilp,execution_time,time_gap) = solve_7(DATA,prob7,LBs7,MAXNODES,TOLGAP,time_limit)
...
for n in dims
	...
	for i1 in 1:4
		for i2 in 1:4
			...
			execution_time += rootnodegap_time
			res  = open("results7.txt","a")
			@printf(res, "%-15s | %-15s | %7.5f | %15.6e | %6d | %7.2f\n",probfile,"FORM(7)",GAP,GUB,totnodes,execution_time)
			close(res)
